/* ============================================================
   HTAT · API del historial compartido — soporta 9 FOTOS
   ...
   La app habla este contrato:
     ...
     GET  /?lineas=1   -> lista de líneas (JSON)
   ...
   CAMBIO RESPECTO A LA VERSION ANTERIOR:
     - Antes solo se guardaban/leían 5 fotos (columnas foto1..foto5);
       las fotos 6 a 9 que mandaba la app se descartaban en silencio.
     - Ahora se guardan/leen hasta 9 (columnas foto1..foto9).
     - Requiere correr primero la migración en D1:
       deploy/sql-migracion-9-fotos.sql
   ============================================================ */
...
async function guardar(env, datos) {
  ...
  if (datos.conservarImagenes) {
    ...
    const actual = await env.DB
      .prepare(
        "SELECT foto1,foto2,foto3,foto4,foto5,foto6,foto7,foto8,foto9,firmaImg " +
        "FROM ots WHERE id = ?"
      )
      .bind(id)
      .first();
